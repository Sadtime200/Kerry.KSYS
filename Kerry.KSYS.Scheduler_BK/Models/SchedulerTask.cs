...
namespace Kerry.KSYS.Scheduler.Models
{
    ...
    public class ScheduleTask
    {
            ...
            CronScheduleBuilder cronBuilder = null)
        {
            ...
            else if (!string.IsNullOrEmpty(cronString))
            {
                Trigger = TriggerBuilder.Create()
                        .WithIdentity(TaskName + "Trigger", TaskType)
        ...
        {
            IJobDetail Job = Sched.GetJobDetail(Key).Result;
            IReadOnlyCollection<ITrigger> Triggers = Sched.GetTriggersOfJob(Key).Result;
            var enumertor = Triggers.GetEnumerator();
            while (enumertor.MoveNext())
            {
                var OnlyTrigger = (ICronTrigger)enumertor.Current;
